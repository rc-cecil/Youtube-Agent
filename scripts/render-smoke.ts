...
try {
  ...
  const edl = {
    ...
    clipEnd: 1.5,
    outputDuration: 1.5,
    eventAnchors: { eventStart: 0, keyMoment: 0.75, payoffEnd: 1.5 },
    durationClass: 'MICRO' as const,
    durationReason: 'A short smoke fixture covering setup, event, and payoff.',
    cropStrategy: 'BLURRED_BACKGROUND' as const,
    cropSubject: null,
    trackingConfidence: 0,
    trackingPolicyVersion: 'tracking-policy-v1',
    fallbackStrategy: 'BLURRED_BACKGROUND' as const,
    trackedSubject: [],
    safeRegions: [],
    hook: {
    ...
    cuts: [
      {
        ...
        outputStart: 0,
        speed: 1,
        reason: 'Smoke render source cut',
      },
    ],
    zooms: [
      {
    ...
    overlays: [
      {
        ...
        x: 0.5,
        y: 0.9,
        reason: 'Smoke render progress',
      },
    ],
    soundEffects: [],
    audioInstructions: { preserveOriginal: true, normalize: true, gainDb: 0, ducking: [] },
    editorialRole: 'HERO' as const,
    editingIntensity: 'MEDIUM' as const,
    concept: 'Verify the complete Remotion media pipeline.',
    title: 'RENDER CHECK',
    titleCandidates: ['RENDER CHECK'],
    ...
    duplicateScore: 0,
    reasoningSummary: 'A deterministic fixture verifies vertical video and preserved audio.',
  };
  const inputProps = {
