...
class SignalProfileDetector implements GameDetector {
  ...
  private generic: GenericGameplayDetector;
  ...
  constructor(
    ...
    this.generic = new GenericGameplayDetector(game, confidence);
  }
...
export function detectorForGame(game: string, confidence: number): GameDetector {
  if (confidence < 0.55) return new GenericGameplayDetector(game, confidence);
  const normalized = game.toLowerCase();
  if (normalized.includes('fifa') || normalized.includes('sports fc'))
    return new FCDetector(confidence);
  if (normalized.includes('grand theft auto')) return new GTADetector(confidence);
  if (normalized.includes('call of duty') || normalized.includes('warzone'))
    return new CODDetector(confidence);
  if (normalized.includes('fortnite')) return new FortniteDetector(confidence);
  return new GenericGameplayDetector(game, confidence);
}
