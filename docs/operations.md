...
Use the YouTube publishing pause on the calendar as the emergency stop. While paused, the API refuses new publication requests and the worker does not advance ordinary upload/schedule/reconcile work. Cancellation requests still run so operators can unwind in-flight schedules. Pausing does not remove schedules already accepted by YouTube; cancel them explicitly or manage them in YouTube Studio.
